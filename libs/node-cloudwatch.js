var crypto = require('crypto'), querystring = require('querystring'), http = require('http');
...
AmazonCloudwatchClient.prototype.queryBuilder = function(command, parameters) {
	...
	for (key in parameters) {
		map[key] = typeof parameters[key] === 'function' ? parameters[key]()
				: parameters[key];
	}
	...
	for (_i = 0, _len = names.length; _i < _len; _i++) {
		name = names[_i];
		query.push(querystring.escape(name) + '='
				+ querystring.escape(map[name]));
	}

	var toSign = 'GET\n' + ('monitoring.us-east-1.amazonaws.com\n') + '/\n'
			+ query.join("&");
	var hmac = crypto
			.createHmac('sha256', process.env['AWS_SECRET_ACCESS_KEY']);
	hmac.update(toSign);
...
AmazonCloudwatchClient.prototype.request = function(action, requestParams,
		callback) {
